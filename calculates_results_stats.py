...
def calculates_results_stats(results_dic):
    ...
    '''
    # removed per grader's suggestion on using .values()
    for i in results_dic.keys():
    
        if results_dic[i][2]: # match
            Y += 1 # - number of matched breed

        if results_dic[i][3]==1: # is dog
            B += 1 # - number of dog images
            if results_dic[i][4]==1: # classified as a dog
                A += 1 # - number of correctly classified dog images
            if results_dic[i][2]==1: # classifier match actual breed
                E += 1 # - number of correctly classified dog breeds
        else: # is NOT a dog
            D += 1 # - number of NON-dog images
            if results_dic[i][4]==0: # classified as not-a-dog
                C += 1 # - number of correctly classified NON-dog images
    ''' # end of removal
    
    # loop through all results_dic values
    # changes made per grader's suggestion
    for value in results_dic.values():
        ...
        if value[3]==1: # is dog
            B += 1 # - number of dog images
            if value[4]==1: # classified as a dog
                A += 1 # - number of correctly classified dog images
            if value[2]==1: # classifier match actual breed
                E += 1 # - number of correctly classified dog breeds
        else: # is NOT a dog
            D += 1 # - number of NON-dog images
            if value[4]==0: # classified as not-a-dog
                C += 1 # - number of correctly classified NON-dog images
...
def pct(a,b):
    if b > 0:
        ...
        print("divider NOT greater than 0")
        return 0  
